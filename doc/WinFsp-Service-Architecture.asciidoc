...
Drive letters created by a service are also affected by Windows DOS device namespace rules. A service-mounted `X:` drive is normally visible to all interactive sessions. If a file system must expose a drive letter only to selected users or sessions, the service should create the drive letter in each selected user's local DOS device namespace, for example by tracking session changes and running or impersonating code in that session to call `DefineDosDeviceW`. A mount security descriptor restricts newly created directory mount points and file-system access, but it does not hide a service-created drive letter from other sessions.
...
The UNC prefix configured with `-u` is a local Windows Network Provider/MUP name. It lets processes on the same computer access the file system as `\\memfs64\share`, and it can also be associated with a local drive letter, but it is not an SMB server export. To make a WinFsp file system available from other computers, create it as a disk file system without a UNC prefix and export the local drive or directory with the Windows Server service, for example by using `NetShareAdd`.
